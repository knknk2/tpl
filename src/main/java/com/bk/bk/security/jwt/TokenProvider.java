...
public class TokenProvider implements InitializingBean {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    if (!StringUtils.isEmpty(secret)) {
      log.warn("Warning: the JWT key used is not Base64-encoded. "
          + "Use `application.security.authentication.jwt.base64-secret` key for optimum security.");
    ...
    this.tokenValidityInMillisecondsForRememberMe = 1000
        * applicationProperties.getSecurity().getAuthentication().getJwt().getTokenValidityInSecondsForRememberMe();
  }
  ...
  public Authentication getAuthentication(String token) {
    ...
    if (uuidObj != null) {
      ...
      log.debug("Claim--> {}", uuid);
    } else {
      log.error("No uuid in token");
    }

    User principal = new User(claims.getSubject(), "", authorities);

    return new SamAuthenticationToken(principal, token, authorities, uuid);
  }
